...
crowdin_sync: true
crowdin_page_id: f199ae24-f711-486d-8e42-3079be87e8cd
---
...
:::info
**Geyser currently supports Minecraft Bedrock <Versions.bedrock /> and Minecraft Java <Versions.java />**.
:::

## Using Geyser with older Minecraft: Bedrock edition versions \{#using-geyser-with-older-minecraft-bedrock-edition-versions}
...
## Using Geyser on older Minecraft: Java edition servers \{#using-geyser-on-older-minecraft-java-edition-servers}

Geyser emulates a <Versions.java /> client, so a Java server must accept users using that version in order for Geyser to work.
...
### Using Geyser-Spigot on a Spigot/Paper server running on 1.16.5 or above \{#using-geyser-spigot-on-a-spigot-paper-server-running-on-1-16-5-or-above}
...
### Using Geyser-Spigot on a Spigot/Paper server running on a version below 1.16.5 \{#using-geyser-spigot-on-a-spigot-paper-server-running-on-a-version-below-1-16-5}
...
### Using Geyser on Fabric/NeoForge servers not on the latest Minecraft version \{#using-geyser-on-fabric-neoforge-servers-not-on-the-latest-minecraft-version}
...
### Using Geyser-Velocity or Geyser-BungeeCord on a proxy \{#using-geyser-velocity-or-geyser-bungeecord-on-a-proxy}

Make sure you update your proxy software to the latest available version, and, if the backend servers are not on <Versions.java />, also install the [ViaVersion](https://github.com/ViaVersion/ViaVersion) plugin on the backend servers. Velocity/BungeeCord support most Minecraft versions, so you can update them regardless of the backend server versions.
