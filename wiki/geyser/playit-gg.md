...
4. Once "Add tunnel" is clicked, it should create a new tunnel, and you are set! Scroll down until you see this:
   ...
   In Geyser config, set the `broadcast-port` to the playit.gg port from the "Allocation" tab. This will fix ping-related issues. The line is commented by default and is located below `compression-level:`. You'll have to remove the hashtag in front of `broadcast-port` to uncomment it.
