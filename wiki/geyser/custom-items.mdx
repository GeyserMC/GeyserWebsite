...
further allows the customization of item behaviour on a per-item-stack basis, such as changing the visual appearance using the item model component,
...
To set up custom items in Geyser, you have to choose how you are going to register your items. The easiest is [using a json mapping file](#json-mappings),
but you can also [use a Geyser extension](#geyser-extensions). Further, a Bedrock edition resource pack must also be supplied to ensure Bedrock players can see custom items.
...
<Tabs groupId="custom-item-implementation">
    <TabItem value="json" label="Json mappings">
        ...
        ```json
        TODO JSON COMPONENT EXAMPLES
        ```
    </TabItem>

    <TabItem value="api" label="Geyser API">
        All supported components can be found in the [JavaItemDataComponents](TODO JAVADOC) class, and are typically built builders.
...
<details>
    ...
    <Tabs groupId="custom-item-implementation">
        <TabItem value="json" label="Json mappings">
            ...
            ```
            This predicate evaluates to "true" when the threshold is higher or equal to double the count value.
