...
crowdin_page_id: 9fc395f8-d2b8-4be3-9d35-8fa573e78c8d
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Minecraft: Java Edition supports creating "custom" items that are based on vanilla items. With Minecraft 1.21.4+, the introduction of item components
further allows the customization of item behavior on a per-item-stack basis, such as changing the visual appearance using the item model component,
and modifying item properties such as the ability to eat or equip an item.
...
would normally require mods to be installed on the Java client.

To set up custom items in Geyser, you have to choose how you are going to register your items. The easiest is using a json mapping file,
but you can also use a Geyser extension which uses the [Geyser API](/wiki/geyser/api/). Further, a Bedrock edition resource pack must also be supplied to ensure Bedrock players can see custom items.

:::warning
Geyser does not convert resource packs from Java Edition, and also does not generate custom item mappings automatically.
However, you can use automatic tools such as [Rainbow](/wiki/other/rainbow/) to make converting content simpler.
:::

:::info
Since the Geyser API version 2.9.3 (Build #1062 and up), the old custom item format (v1) is deprecated and is no
longer being updated. Old docs can still be found [here](/wiki/geyser/custom-items-old)
:::
...
- `Java item component`: On Java Edition since 1.21.4+, items have their properties and behavior defined using item components.
Every Java item has a set of default components, which can be overridden to change how an item looks or functions.
- `Bedrock item component`: Custom Bedrock items also have components that determine item behavior, similar to Java Edition.
However, these components cannot be changed at runtime for item stacks and have to be pre-defined when the Bedrock client joins.
...
- `bedrock_identifier`: This identifier is used for the custom Bedrock item (and also in e.g. attachables in Bedrock resource packs), and cannot be shared with **any** other custom item definition.
...
<Tabs groupId="custom-item-implementation">
    ...
    <TabItem value="api" label="Definitions: Geyser API">
        Custom item definitions are registered using the [`GeyserDefineCustomItemsEvent`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/event/lifecycle/GeyserDefineCustomItemsEvent.html).
...
This value corresponds to icon shorthands defined in the `item_texture.json` file.
...
- `protection_value`: Determines how many armor protection points should be shown when this item is worn. This is purely visual,
...
for a list of possible values. Creating custom creative groups is not currently supported by Geyser.
...
<Tabs groupId="custom-item-implementation">
    ...
    <TabItem value="api" label="Geyser API">
        In the API, Bedrock item options are defined using the [`CustomItemBedrockOptions#Builder`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/item/custom/v2/CustomItemBedrockOptions.Builder.html).
...
Therefore, any modifications to the base Java item's data components must be specified in mapping files.
It is expected that a custom item will *always* have these components sent by the server when the custom item definition
is used. For different sets of components, multiple custom item definitions with predicates should be used.
...
:::warning
There are some limitations about which components can be used together!
For example, Bedrock currently [does not support](https://bugs.mojang.com/browse/MCPE/issues/MCPE-176931) wearable stackable items.
You can remove conflicting default components as a temporary fix when using equippable items (e.g. horse armor) as base items.
:::
...
- `minecraft:use_effects`
    - Due to Bedrock limitations, the `can_sprint` property cannot be translated. Further, this component will only be translated
        when the base item is either able to shoot arrows, throw projectiles, or is edible (or made edible with the food component).
...
<Tabs groupId="custom-item-implementation">
    ...
    <TabItem value="api" label="Geyser API">
        All supported components can be found in the [`JavaItemDataComponents`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/item/custom/v2/component/java/JavaItemDataComponents.html) class, and are typically built with builders.
...
<Tabs groupId="custom-item-implementation">
    ...
    <TabItem value="api" label="Geyser API">
        The [`CustomItemDefinition#Builder`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/item/custom/v2/CustomItemDefinition.Builder.html)
...
<Tabs groupId="custom-item-implementation">
    ...
    <TabItem value="api" label="Geyser API">
        All condition predicates can be found in the [`ItemConditionPredicate`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/predicate/item/ItemConditionPredicate.html) interface. Examples:
...
<details>
    ...
    <Tabs groupId="custom-item-implementation">
        ...
        <TabItem value="api" label="Geyser API">
            Item match predicates can be found in the [`ItemMatchPredicate`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/predicate/item/ItemMatchPredicate.html)
            interface, additionally to generic match predicates in the [`MatchPredicate`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/predicate/MatchPredicate.html) interface.
...
<details>
    ...
    <Tabs groupId="custom-item-implementation">
        <TabItem value="json" label="Json mappings">
            ...
            ```
            This predicate evaluates to "true" when the normalized count (count / max count; between 0 and 1) is equal to
            or higher than 0.5.
        ...
        <TabItem value="api" label="Geyser API">
            Range dispatch predicates are found in the [`ItemRangeDispatchPredicate`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/predicate/item/ItemRangeDispatchPredicate.html) interface.
...
the provided predicates, where possible. Chaining predicates manually or creating custom predicates results in Geyser being
unable to cache results!
...
The [`GeyserItemDataComponent`](https://repo.opencollab.dev/javadoc/maven-snapshots/org/geysermc/geyser/api/latest/.cache/unpack/org/geysermc/geyser/api/item/custom/v2/component/geyser/GeyserItemDataComponents.html) class contains all additional components available for modded items:
