...
4. Open your Geyser config, located in `/plugins/Geyser/config.yml`, and find the following:
    ...
    If you enable `clone-remote-port`, the port will be overridden and the ViaProxy port is used, but be careful,
    as this only works when using [Nethernet External Signalling](/wiki/geyser/nethernet/setup#nethernet-external-signalling)
    as the Nethernet TCP port cannot collide with the Java one.
